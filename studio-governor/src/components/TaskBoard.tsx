      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {groupedTasks.active.length > 0 && (
          <div className="space-y-2">
            <div className="text-[9px] font-bold text-primary/70 uppercase px-1">
              Active Now
            </div>
            {groupedTasks.active.map((task) => (
              <div
                key={task.id}
                className="p-3 rounded-lg bg-primary/5 border border-primary/20 shadow-lg shadow-primary/5"
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[8px] text-primary/50 font-mono">
                    ID: {task.id.slice(0, 8)}
                  </span>
                </div>
                <p className="text-[10px] text-gray-200 mb-2 leading-tight">
                  {task.text}
                </p>
        ...
        {/* Pending Column */}
        <div className="space-y-2">
          <div className="text-[9px] font-bold text-gray-500 uppercase px-1">
            Backlog
          </div>
        ...
        {groupedTasks.done.length > 0 && (
          <div className="space-y-2 pt-2 border-t border-white/5">
            <div className="text-[9px] font-bold text-gray-600 uppercase px-1">
              Archive
            </div>
            {groupedTasks.done.slice(0, 3).map((task) => (
              <div
                key={task.id}
                className="flex items-center justify-between p-2 rounded bg-white/5 border border-transparent opacity-60"
              >
